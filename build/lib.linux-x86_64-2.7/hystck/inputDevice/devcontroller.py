...
from hystck.core.guest import Guest
from hystck.botnet.common.loggerbase import LoggerBase
from hystck.botnet.common.runtimequery import RuntimeQuery
from hystck.botnet.common.threadmanager import ThreadManager
from hystck.botnet.net.meta.tcpclientbase import TcpClientBase
from hystck.botnet.net.meta.tcpserverbase import TcpServerBase
from hystck.botnet.net.netutility import NetUtility
...
def _unicode_to_str(data):
    ...
    if isinstance(data, dict):
        return {_unicode_to_str(key): _unicode_to_str(value) for key, value in data.iteritems()}
...
class IRelativeMouseMethods(object):
    __metaclass__ = ABCMeta
...
class DevDriverAdapter(IKeyboardMethods, IRelativeMouseMethods):
    __metaclass__ = ABCMeta
...
class LinuxAbstractAdapter(DevDriverAdapter):
    __metaclass__ = ABCMeta
...
#        self.translator.load_template('/etc/hystck/set1-basic.conf')
...
class LinuxXServerAdapter(LinuxAbstractAdapter, ILayoutSwitcher, LoggerBase):
    ...
    def __init__(self):
        ...
        self.translator.load_template("/etc/hystck/set1-de.conf")
        self.translator.generate_mapping()
...
class XTLayoutTranslator(DevTranslator):
    ...
    def load_template(self, keymap_file="/etc/hystck/set1-de.conf"):
...
class DevControlServer(TcpServerBase, LoggerBase, RuntimeQuery, ThreadManager):
    ...
    def dbg_dump_mapper(self, sock):
        p = cPickle.dumps(self.driver.translator.mapper)  # apparently json cannot dump byte-arrays, need to pickle it
        # msg = json.dumps(p)
...
class DevControlClient(TcpClientBase):
    ...
    def connect(self, guest, port=DEV_PORT, timeout=None):
        ...
        :type guest: hystck.core.guest.Guest | str
        :type port: int
