...
def setguestboottimeoffsetlocal(guestname, boottime, guestclocktypeisutc=False):
    ...
    tn = dt.datetime.now()
    bt = dt.datetime.strptime(boottime, "%Y-%m-%d %H:%M:%S")
    ...
    c.set(key='offset', value="variable")
    if guestclocktypeisutc:
        c.set(key='basis', value="utc")
    else:
        c.set(key='basis', value="localtime")
    c.set(key='adjustment', value=str(tds))
    xt = et.ElementTree(x)
