    ...
    for i, packet in enumerate(cap):
        if 'req_parameter' in packet.smtp.field_names and 'req_command' in packet.smtp.field_names:
            if packet.smtp.req_command == 'MAIL':
                _print_source_destination(packet)
            ...
            if packet.smtp.req_command == 'DATA':
                ...
                packet = cap[i + 2]
                mail_content = _parse_mail_content(packet.smtp)
    ...
    for i, packet in enumerate(cap):
        if int(str(packet.ipp.request_id)) == 0x2:
            if 'data' in packet.ipp.field_names:
                ...
                print_job = _parse_ipp_content(packet.ipp)
...
def _check_requirements(requirements, counter):
...
def main():
    ...
    requirements = _parse_config(args.config)

    _parse_mails(args.pcap)
    _parse_smb(args.pcap)
    _parse_ipp(args.pcap)
    ...
    _check_requirements(requirements, COUNTER)
